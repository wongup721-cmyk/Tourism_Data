...
def cal_eval(y_real, y_pred):
    ...
    return df_eval
df = pd.read_csv(r'C:\Users\29324\Desktop\最终处理(最终版)\timenet +tsmixer\timenet\data\agent剔除疫情.csv')
...
# 保存评估指标到Excel文件
output_filename = input("请输入要保存的Excel文件名(不需要输入.xlsx后缀): ")
save_path = r'C:\Users\29324\Desktop\最终处理(最终版)\timenet +tsmixer\timenet\result\itransformer'
df_eval.to_excel(f"{save_path}\{output_filename}.xlsx")
print(f'评估指标已保存到 {save_path}\{output_filename}.xlsx 文件中。')
