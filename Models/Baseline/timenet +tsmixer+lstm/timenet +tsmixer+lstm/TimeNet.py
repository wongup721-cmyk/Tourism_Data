...
def cal_eval(y_real, y_pred):
    ...
    mse = mean_squared_error(y_real, y_pred)  # 直接计算均方误差
    rmse = np.sqrt(mse)                     # 手动计算均方根误差
    mae = mean_absolute_error(y_real, y_pred)
    mape = mean_absolute_percentage_error(y_real, y_pred) * 100  # 注意数据中不能有0值
    ...
    return df_eval
df = pd.read_csv(r'C:\Users\29324\Desktop\最终处理(最终版)\timenet +tsmixer\timenet\data\旅游人数.csv')
# 注意多变量情况下，目标变量必须为最后一列
data_dim = df[df.columns.drop('date')].shape[1]  # 一共多少个变量
data_target = df['N-tourist']  # 预测的目标变量
...
data_stamp = time_features(df_stamp, timeenc=1, freq='W')  # 这一步很关键，注意数据的freq
...
class Config:
    def __init__(self):
        ...
        self.n_heads = 16                       # 多头注意力头数
        self.dropout = 0.5                      # 丢弃率
        self.e_layers = 8                      # 编码器块的数量
        ...
        self.task_name = 'long_term_forecast'  # 模型的任务，一般不动但是必须这个参数
config = Config()


model_type = 'TimesNet'
net = TimesNet.Model(config).to(device)
...
# 获取实际可预测的日期范围
# window 是输入序列长度，length_size 是预测序列长度
start_idx = window + length_size - 1  # 考虑滑动窗口造成的数据损失
test_dates = df['date'].iloc[int(train_set * data_length) + start_idx:].values

# 确保日期长度与预测值匹配
if len(test_dates) > len(pred):
    test_dates = test_dates[:len(pred)]
elif len(test_dates) < len(pred):
    pred = pred[:len(test_dates)]
    true = true[:len(test_dates)]
...
# 保存到Excel的不同sheet中
excel_path = r'C:\Users\29324\Desktop\最终处理(最终版)\timenet +tsmixer\timenet\result\timesnet\旅游人数4.xlsx'
with pd.ExcelWriter(excel_path) as writer:
    result_df.to_excel(writer, sheet_name='预测结果', index=False)
    df_eval.to_excel(writer, sheet_name='评估指标')

print(f'预测结果和评估指标已保存到: {excel_path}')
