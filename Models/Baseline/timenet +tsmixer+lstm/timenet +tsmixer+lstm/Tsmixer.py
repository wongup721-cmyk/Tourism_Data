...
from models import TimesNet, TSMixer
...
def cal_eval(y_real, y_pred):
    ...
    return df_eval
df = pd.read_csv(r'C:\Users\29324\Desktop\最终处理(最终版)\timenet +tsmixer\timenet\data\mrmr剔除疫情.csv')
...
class Config:
    def __init__(self):
        ...
        self.e_layers = 5  # 编码器块的数量
        self.d_layers = 2  # 解码器块的数量
        self.d_ff = 4  # 全连接网络维度
        self.factor = 4  # 注意力因子
...
model_type = 'TSMixer'
net = TSMixer.Model(config).to(device)
...
# 保存到Excel的不同sheet中
excel_path = r'C:\Users\29324\Desktop\最终处理(最终版)\timenet +tsmixer\timenet\result\tsmixer\mrmr剔除疫情1.xlsx'
